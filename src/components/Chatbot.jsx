...
function Chatbot({ isDropdown = false, onClose = null }) {
  ...
  const [setShowWelcomeAnimation] = useState(true);
  ...
  }, [isOpen, setShowWelcomeAnimation]);
  ...
  return (
    <div
      className={isDropdown ? "w-full" : "fixed bottom-4 right-4"}
      style={isDropdown ? {} : { zIndex: 1001 }}
    >
      {!isDropdown && (
      ...
      {isOpen && (
        <div
          ...
          className={`${
            isDropdown
              ? "h-[400px] w-[320px] sm:w-[380px] rounded-3xl"
              : "absolute bottom-20 right-0 h-[400px] w-[320px] sm:w-[380px] rounded-3xl"
          } overflow-hidden shadow-2xl animate-slide-in-up backdrop-blur-xl border border-white/10`}
          style={{
